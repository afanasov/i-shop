    ...
    if (isset($action))
    {
        switch ($action) 
        {
            case 'accept':
                ...
                {                
                    $update = mysql_query("UPDATE table_reviews SET moderat='1' WHERE reviews_id = '$id'",$link) or die(mysql_error($link));                                         
                }else{ $msgerror = 'У вас нет прав на одобрение отзывов!';}
            break;
        
            case 'delete': 
                if($_SESSION['accept_reviews'] == '1')
                {                
                    $delete = mysql_query("DELETE FROM table_reviews WHERE reviews_id = '$id'",$link) or die(mysql_error($link));                                         
                }else{ $msgerror = 'У вас нет прав на удаление отзывов!';}
            break;        
        }     
    }              
?>
    ...
    <body>
        <div id="block-body">
            <?php
                ...
                $all_count = mysql_query("SELECT * FROM table_reviews",$link) or die(mysql_error($link)) or die(mysql_error($link));
                $all_count_result = mysql_num_rows($all_count);
                
                $accept_count = mysql_query("SELECT * FROM table_reviews WHERE moderat = '1'",$link) or die(mysql_error($link)) or die(mysql_error($link));
                $accept_count_result = mysql_num_rows($accept_count); 
                
                $no_accept_count = mysql_query("SELECT * FROM table_reviews WHERE moderat = '0'",$link) or die(mysql_error($link)) or die(mysql_error($link));
            ...
            <div id="block-content">
                    ...
                    <ul id="options-list">
                        ...
                        <li><a id="select-links" href="#"><? echo $sort_name; ?></a>
                            <ul id="list-links-sort">
                                <li><a href="reviews.php?sort=accept">Проверенные - <strong><?php echo $accept_count_result;?></strong></a></li>
                                <li><a href="reviews.php?sort=no-accept">Не проверенные - <strong><?php echo $no_accept_count_result;?></strong></a></li>
                                <li><a href="reviews.php?sort=default">Без сортировки <strong></strong></a></li>
                            </ul>
                        </li>
                    </ul>
                </div>
                ...
                <div id="block-info">
                    ...
                    <?php
                        ...
                        $count = mysql_query("SELECT COUNT(*) FROM table_reviews",$link) or die(mysql_error($link));
                        ...
                        $result = mysql_query("SELECT * FROM table_reviews,table_products WHERE table_products.products_id = table_reviews.products_id ORDER BY $sort LIMIT $start, $num",$link) or die(mysql_error($link));
