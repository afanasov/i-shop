       ...
       switch ($action) 
        {
            case 'delete':
                ...
                {
                    $delete = mysql_query("DELETE FROM reg_user WHERE id = '$id'",$link) or die(mysql_error($link)); 
                }else{ $msgerror = 'У вас нет прав на удаление клиентов!';}
            break;        
        } 
    }   
?>
    ...
    <body>
        <div id="block-body">
            <?php
                ...
                $all_client = mysql_query("SELECT * FROM reg_user",$link) or die(mysql_error($link));
            ...
            <div id="block-content">
                ...
                <?php                   
                    ...
                    if($_SESSION['view_clients'] == '1')
                    {
                        ...
                        $count = mysql_query("SELECT COUNT(*) FROM reg_user",$link) or die(mysql_error($link));
                        ...
                        if ($temp[0] > 0)   
                        { 
                            $result = mysql_query("SELECT * FROM reg_user ORDER BY id DESC LIMIT $start, $num",$link) or die(mysql_error($link));
                         ...
                         if ($page != 1) $pervpage = '<li><a class="pstr-prev" href="clients.php?page='. ($page - 1) .'" />Назад</a></li>';

                            if ($page != $total) $nextpage = '<li><a class="pstr-next" href="clients.php?page='. ($page + 1) .'"/>Вперёд</a></li>';
                    ...
                    }else{ $msgerror = 'У вас нет прав на просмотр данной страницы!';}    
                ?>
            </div>
        </div>
