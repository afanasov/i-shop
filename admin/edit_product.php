<?php
    ...
    $_SESSION['urlpage'] = "<a href='index.php' >Главная</a> / <a href='tovar.php' >Товары</a> / <a href='#' >Редактирование товара</a>";    
    ...
    if (isset($action))
    {
	    ...
	    case 'delete':
                ...
                }else {$msgerror = 'У вас нет прав на изменения данного товара!';}
	    break;	
        }
    }    
    //Измен. товара
    if ($_POST["submit_save"])
    {
        if($_SESSION['edit_tovar'] == '1')
        {
            ...
            {                                
                $result = mysql_query("SELECT * FROM category WHERE id='{$_POST["form_category"]}'",$link) or die(mysql_error($link));
            ...
            if (empty($_POST["upload_image"]))
                {   
                    require 'actions/upload-image.php';
                    unset($_POST["upload_image"]);           
                }       
                if (empty($_POST["galleryimg"]))
                {        
                    require 'actions/upload-gallery.php';
...
<html>
    <head>
        ...
        <title>Панель управления - Редактор товаров </title>
    </head>
    <body>
        <div id="block-body">
            <?php
	...
	$result = mysql_query("SELECT * FROM table_products WHERE products_id='$id'",$link) or die(mysql_error($link));
        ...
        If (mysql_num_rows($result) > 0)
        {
            ...
            {    
                ...
                ';        
                $category = mysql_query("SELECT * FROM category",$link) or die(mysql_error($link));   
                ...
                if  (strlen($row["image"]) > 0 && file_exists("../uploads_images/".$row["image"]))
                {
                    ...
                    echo '
                        ...
                        <div id="baseimg">
                            ...
                            <a href="edit_product.php?id='.$row["products_id"].'&img='.$row["image"].'&action=delete" ><img src="images/action_delete.png"/></a>
                        </div>
                    ';   
                }else
                {  
                ...
                $query_img = mysql_query("SELECT * FROM uploads_images WHERE products_id='$id'",$link) or die(mysql_error($link));
                If (mysql_num_rows($query_img) > 0)
                {
                    ...
                    do
                    {
                        ...
                        if($_SESSION['edit_tovar'] == '1')
                            {                        
                                echo '
                                    <a class="del-img" img_id="'.$result_img["id"].'" ><img src="images/action_delete.png"/></a>       
                                ';
                            }{$msgerror = 'У вас нет прав на изменения данного товара!';}
                        echo '</li>';
                ...
                if ($row["availability"] == '1') $checked5 = "checked";
                echo ' 
                    ...
                    <ul id="chkbox">
                        ...
                        <li><input type="checkbox" name="availability" id="availability" '.$checked5.' /><label for="availability" >Наличие</label></li>
                    </ul>
                    <div id="datatime">
                        <p align="right" >Дата и время добавления товара: <strong>'.$row["datatime"].'</strong>  </p>
                    </div>
