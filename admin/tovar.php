<?php
    ...
    if (isset($action))
    {
        ...
        switch ($action) 
        {
            ...
            {
                $delete = mysql_query("DELETE FROM table_products WHERE products_id = '$id'",$link) or die(mysql_error($link));  
            }   
            else { $msgerror = 'У вас нет прав на удаление данного товара!';}
	break;
...
<html>
    <head>
        ...
        <title>Панель управления - Обзор товаров</title>
    </head>
    <body>
        <div id="block-body">
            <?php
                ...
                $all_count = mysql_query("SELECT * FROM table_products",$link) or die(mysql_error($link));
            ...
            <div id="block-content">
                ...
                <ul id="options-list">
                        ...
                        <li><a id="select-links" href="#"><? echo $cat_name; ?></a>
                            <div id="list-links" >
                                <ul>
                                    ...
                                    <?php
                                        $result1 = mysql_query("SELECT * FROM category WHERE type='mobile'",$link) or die(mysql_error($link));
                                ...
                                <ul>
                                    ...
                                    <?php
                                        $result1 = mysql_query("SELECT * FROM category WHERE type='notepad'",$link) or die(mysql_error($link));
                                        If (mysql_num_rows($result1) > 0)
                                        {
                                            $row1 = mysql_fetch_array($result1);
                                            do
                    ...
                    <?php
                        ...
                        $start = $page * $num - $num;
                        // Вывод товаров в основном блоке
                        if ($temp[0] > 0)   
                        {
                            $result = mysql_query("SELECT * FROM table_products $cat ORDER BY products_id DESC LIMIT $start, $num",$link) or die(mysql_error($link));
