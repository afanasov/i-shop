...
//проверка существования переменной и добавления в случае отсутствия
...
$title = mysql_query("SELECT`type_tovara` FROM table_products WHERE type_tovara='$type' AND visible='1'",$link)or die(mysql_error($link));
...
<html>
    <head>
        ...
        <title>Интернет магазин просмотр <?php echo $type_index; ?></title>
    </head>
    <body>
        <div id="block-body">
            <?php //Шапка сайта
          ...
          if (!empty($cat) && !empty($type)) 
            {
                ...
                {
    //Проверка на не пустату модели и категории
                    if (!empty($type))
                        {
                ...
                // запрос в БД               
                $result = mysql_query("SELECT * FROM table_products WHERE visible='1' $querycat ORDER BY $sorting $qury_start_num ",$link) or die(mysql_error($link));
              ...
              <ul id="block-tovar-list">
                ...
                if ($page != $total) $pstr_next = '<li><a class="pstr-next" href="view_cat?page='.($page + 1).'">&gt;</a></li>';
