...
<div id="block-random-tovar">
    <ul>
        <?php
            ...
            $query_random = mysql_query("SELECT DISTINCT * FROM table_products WHERE visible='1' ORDER by RAND() LIMIT 4",$link) or die(mysql_error($link));  
            ...
            If (mysql_num_rows($query_random) > 0)
            {
                ...
                {
                    // Выбор отзывов    
                    $query_reviews = mysql_query("SELECT * FROM table_reviews WHERE products_id = {$res_query["products_id"]} AND moderat='1'",$link) or die(mysql_error($link));  
                    ...
                    echo '
                        <li>
                            ...
                            <p class="random-reviews">Просмотры: '.$res_query["count"].'</p>
