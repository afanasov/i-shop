<?php 
    define('myeshop', true);
    //header('Location http://asdat.info/', true, 301);// не работает редирект
    session_start(); 
    ...
    case 'oneclick':    
        echo '          
            <div id="block-step">
                <div id="name-step">
                    <ul>
                        <li><a class="active" href="cart.php?action=oneclick">1. Корзина товаров</a></li>
        ...
        $result = mysql_query("SELECT * FROM cart,table_products WHERE cart.cart_ip = '{$_SERVER['REMOTE_ADDR']}' AND table_products.products_id = cart.cart_id_product",$link) or die(mysql_error($link));
        ...
        If (mysql_num_rows($result) > 0)
        {
            ...
            echo '
                ...
                <div id="tovar'.$row["cart_id"].'" class="price-product">
                    <h5><span class="span-count" >'.$row["cart_count"].'</span> x <span>'.group_numerals($row["cart_price"]).' грн</span></h5>
    ...
    case 'confirm':        
        ...
        if ( $_SESSION['auth'] != 'yes_auth' ) 
        {
            echo '
                ...
                <li><label for="order_email"><span>*</span>E-mail</label><input type="text" name="order_email" id="order_email" value="'.$_SESSION["order_email"].'" /><span class="order_span_style" >Пример: ivanov@gmail.com</span></li>
                <li><label for="order_phone"><span>*</span>Телефон</label><input type="text" name="order_phone" id="order_phone" value="'.$_SESSION["order_phone"].'" /><span class="order_span_style" >Пример:  123 456 78 90</span></li>
                <li><label class="order_label_style" for="order_address"><span>*</span>Адрес<br /> доставки</label><input type="text" name="order_address" id="order_address" value="'.$_SESSION["order_address"].'" /><span>Пример: г. Харьков,<br /> ул Сумская дом 1</span></li>
            ';
        }
        echo '
                <li><label class="order_label_style" for="order_note">Примечание</label><textarea name="order_note"  >'.$_SESSION["order_note"].'</textarea><span>Уточните информацию о заказе.</span></li>
                 ...
                 <p align="right" ><a href="cart.php?action=completion"><input type="submit" name="submitdata" id="confirm-button-next" value="Далее" /></a></p> 
                <!-- <p align="right" class="button-next" ><a name="submitdata" href="cart.php?action=completion" name="submitdata">Далее</a></p> -->
            </form>
        ';
            ...
            <div id="block-step">
                <div id="name-step">
                    <ul>
                        ...
                        <li><a  href="cart.php?action=confirm">2. Контактная информация</a></li>
                        <li><span>&rarr;</span></li>
                        <li><a  class="active" href="cart.php?action=completion">3. Завершение</a></li>
                    </ul>
                </div>
                <p>Шаг 3 из 3</p>                
            </div>          
        <h3 id="list-info_itog">Итоговая информация</h3>
        ';
...
if ( $_SESSION['auth'] == 'yes_auth' ) 
{
    ...
    echo '
        <h2 class="itog-price" align="right">Итого: <strong>'.group_numerals($itogpricecart).'</strong> грн</h2>
                ...
                <div class="block-list-cart">        
                    ...
                    <div class="count-cart">
                        <ul class="input-count-style">
                            ...
                            <li>
                                <p align="center"><input id="input-id'.$row["cart_id"].'" id="" class="count-input" maxlength="3" type="text" value="'.$row["cart_count"].'" /></p>
                            </li>
                            <li>
                                <p align="center" iid="'.$row["cart_id"].'" class="count-plus">+</p>
                            </li>
